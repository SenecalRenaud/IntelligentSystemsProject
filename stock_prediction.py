...
#Function for multistep and multivariate prediction, it re-uses the code in processData except for the Testing section
# k = lookup_days
def multistepMultivariatePredicting(ticker, startDate=None, endDate=None, testToTrainRatio=0.2,
                                    lookup_days=1, storeDataLocally=False, splitByDate=True, scale=False):
    if isinstance(ticker, str):
        # load data from yahoo finance library
        data = yf.download(ticker, startDate, endDate, progress=False)
    elif isinstance(ticker, pd.DataFrame):
    ...
    #create prediction arrays that will be used for the multistep prediction
    x_prediction = x_test[-1:, :, :]
    y_prediction = y_test[-1]
    #predicted closing prices (result) array
    future_y = []

    for i in range(lookup_days):
        #make previous day closing price last of the testing data for next day
        x_prediction = np.append(x_prediction[:, 1:, :], y_prediction.reshape(1, 1, 1), axis=1)
        #predict next closing price
        y_prediction = model.predict(x_prediction)
        #append closing price to results
        future_y.append(y_prediction.flatten()[0])
    ...
    if scale:
        y_test = np.squeeze(result["column_scaler"]["adjclose"].inverse_transform(np.expand_dims(y_test, axis=0)))
        predicted_prices = np.squeeze(data["column_scaler"]["adjclose"].inverse_transform(future_y))
    # Reverse the scaling
